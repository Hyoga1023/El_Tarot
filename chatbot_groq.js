...
Usa tono sabio pero directo. No divagues.`;
      ...
      messages: [
        {
          role: "system",
          content:
            "Eres un tarotista sabio. Responde siempre en segunda persona, de forma concisa y directa. Máximo 250 palabras por respuesta.",
        },
        {
          role: "user",
          content: prompt,
    ...
    if (!response.ok) {
      ...
      return `Error en el oráculo (${response.status}). ${
        errorData.error?.message || "Intenta de nuevo."
      }`;
    }
...
function limpiarApiKey() {
  localStorage.removeItem("groq_api_key");
  console.log(
    "API Key eliminada. Recarga la página para introducir una nueva."
  );
}
